    ...
    <main style={{ fontFamily: '"Helvetica Neue", Arial, sans-serif', background: '#f7f5f0', color: '#2e342b', minHeight: '100vh' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', gap: 28, alignItems: 'center' }}>
          <a href="/login" style={{ color: 'rgba(255,255,255,0.92)', fontWeight: 600, fontSize: 14, textDecoration: 'none' }}>Inloggen</a>
      ...
      <header style={{ position: 'relative', height: '100vh', minHeight: 620, display: 'flex', alignItems: 'flex-end', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, width: '100%', maxWidth: 1200, margin: '0 auto', padding: '0 24px 90px', color: '#fff' }}>
          <span style={{
            ...
            Platform voor ZZP-koks en horeca
          </span>
          <h1 style={{ fontSize: 'clamp(42px, 7vw, 84px)', fontWeight: 800, lineHeight: 1.02, letterSpacing: -1.5, maxWidth: '13ch', marginBottom: 22 }}>
            De juiste chef, <span style={{ color: '#e4e9dd' }}>op het juiste moment.</span>
          </h1>
          <p style={{ fontSize: 18, maxWidth: '46ch', opacity: 0.92, marginBottom: 34 }}>
            Vind geverifieerde zzp-koks voor jouw shift, of vind als kok de leukste keukens van Nederland. Snel, veilig en zonder gedoe.
          </p>
      ...
      <section style={{ padding: '90px 24px', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', maxWidth: 640, margin: '0 auto 56px' }}>
          <span style={{ color: '#5f7052', fontWeight: 800, fontSize: 13, letterSpacing: 2.5, textTransform: 'uppercase' }}>Zo werkt het</span>
          <h2 style={{ fontSize: 'clamp(30px, 4vw, 46px)', fontWeight: 800, letterSpacing: -1, margin: '10px 0 14px' }}>In drie stappen geregeld</h2>
          <p style={{ color: '#6b7268', fontSize: 17 }}>Geen uitzendbureau, geen eindeloos zoeken. ChefShift brengt keukens en koks direct bij elkaar.</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 24 }}>
          {[
            { n: '1', t: 'Maak je profiel', d: 'Registreer als horecazaak of als zzp-kok. Verificatie met KvK en HACCP inbegrepen.' },
            { n: '2', t: 'Plaats of vind een shift', d: 'Zet een shift online of reageer direct op een opdracht die bij jou past.' },
            { n: '3', t: 'Werk en betaal veilig', d: 'Afspraken en betalingen lopen via het platform, met iDEAL. Zekerheid voor beide kanten.' },
          ].map((e) => (
      ...
      <section style={{ padding: '0 24px 90px', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', maxWidth: 640, margin: '0 auto 56px' }}>
          <span style={{ color: '#5f7052', fontWeight: 800, fontSize: 13, letterSpacing: 2.5, textTransform: 'uppercase' }}>Waarom ChefShift</span>
          <h2 style={{ fontSize: 'clamp(30px, 4vw, 46px)', fontWeight: 800, letterSpacing: -1, margin: '10px 0 14px' }}>Gemaakt voor de horeca</h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 28 }}>
          {[
            { icon: '⚡', t: 'Snelle matching', d: 'Een shift geplaatst? Gemiddeld binnen 2 uur de eerste reacties van beschikbare koks.' },
            { icon: '✅', t: 'Geverifieerde koks', d: 'Elke kok wordt gecontroleerd op KvK-inschrijving en HACCP-certificering.' },
            { icon: '🔒', t: 'Veilig betalen', d: 'Betalingen via iDEAL, vastgelegd in het platform. No cure, no pay.' },
          ].map((c) => (
      ...
      <section style={{ padding: '0 24px 90px', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ position: 'relative', borderRadius: 26, overflow: 'hidden', padding: '80px 40px', textAlign: 'center', color: '#fff' }}>
          ...
          <div style={{ position: 'relative', zIndex: 2 }}>
            <h2 style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 800, letterSpacing: -1, marginBottom: 14 }}>Klaar om je eerste shift te vullen?</h2>
            <p style={{ opacity: 0.9, marginBottom: 30, fontSize: 17 }}>Maak vandaag nog een gratis account aan.</p>
      ...
      <footer style={{ background: '#2e342b', color: 'rgba(255,255,255,0.75)', padding: '60px 24px 30px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <p style={{ maxWidth: 360, marginBottom: 40 }}>Het platform voor zzp-koks en horecazaken in Nederland.</p>
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.12)', paddingTop: 24, fontSize: 13.5, textAlign: 'center' }}>
            © 2026 ChefShift — Gemaakt voor de horeca.
          </div>
        </div>
      </footer>
    </main>
